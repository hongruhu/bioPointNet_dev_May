...
adata = sc.read_h5ad("/group/gquongrp/workspaces/hongruhu/MIL/neural_organoid/scPN_modeling/hnoca_intersected_union1000hvg.h5ad")
# AnnData object with n_obs × n_vars = 1641349 × 1766
adata.obs.bio_sample.value_counts()

sample_key = "bio_sample"
adata.obs[sample_key].unique() # 282 samples

metadata = adata.obs.copy()
metadata.organoid_age_days.value_counts().sort_index()
# 7d - 192d, 50 time points.


# distribution plot of organoid_age_days for cells
fig = plt.figure(figsize=(10, 5))
sns.histplot(metadata.organoid_age_days, kde=True)
sns.despine()
plt.show()
fig.savefig("organoid_age_days.pdf", dpi=150)
fig = plt.figure(figsize=(15, 7))
sns.countplot(data=metadata, x='organoid_age_days', order=metadata['organoid_age_days'].value_counts().index)
plt.xticks(rotation=90)
plt.xlabel('Organoid Age Days')
plt.ylabel('Count of Cells')
plt.title('Count of Cellsfor Different Organoid Age Days')
sns.despine()
plt.show()
fig.savefig("Cell_count_organoid_age_days.pdf", dpi=150)
fig = plt.figure(figsize=(15, 7))
sns.countplot(data=metadata, x='organoid_age_days', order=sorted(metadata['organoid_age_days'].unique()))
plt.xticks(rotation=90)
plt.xlabel('Organoid Age Days')
plt.ylabel('Count of Cells')
plt.title('Count of Cells for Different Organoid Age Days')
sns.despine()
plt.show()
fig.savefig("Cell_count_organoid_age_days_sorted.pdf", dpi=150)


# Group by bio_sample and get the first occurrence of each bio_sample
metadata_grouped = metadata.groupby('bio_sample').first().reset_index()

# Count plot of bio_sample size for different organoid_age_days sorted by x-axis from small to large
fig = plt.figure(figsize=(15, 7))
sns.countplot(data=metadata_grouped, x='organoid_age_days', order=sorted(metadata_grouped['organoid_age_days'].unique()))
plt.xticks(rotation=90)
plt.xlabel('Organoid Age Days')
plt.ylabel('Count of Bio Samples')
plt.title('Count of Bio Samples for Different Organoid Age Days')
sns.despine()
plt.show()
fig.savefig("bio_sample_count_organoid_age_days_sorted.pdf", dpi=150)





metadata.index = ['cell_' + str(i) for i in range(metadata.shape[0])]
...
metadata['organoid_age_days_rank'] = metadata['organoid_age_days'].rank(method='dense') - 1
metadata.organoid_age_days_rank.value_counts().sort_index()


Ys_df = metadata[['organoid_age_days_rank']].copy()
Ys_df
...
for epoch in range(epochs):
    ...
    if loss_val_avg < best_val_loss:
        ...
        torch.save(pn_cls, "best_model_HNOCA_from_Gene.pt")
        patience_counter = 0
...
pn_cls_checkpoint = torch.load("best_model_HNOCA_from_Gene.pt")
...
# organoid_age_days_rank    0.96052
true_label_val_df.corrwith(pred_label_val_df, method="pearson")
# organoid_age_days_rank    0.970783
# correlation between true and predicted labels
...
# organoid_age_days_rank    0.995861
true_label_tr_df.corrwith(pred_label_tr_df, method="pearson")
# organoid_age_days_rank    0.998635
...
len(attention_mtx_list)     # 107
len(instance_level_list)    # 107
...
metadata_tr.to_csv("metadata_tr_HNOCA.csv")
attention_mtx_raw_df.to_csv("attention_mtx_HNOCA.csv")
...
hdbca.obs.sample_id.value_counts()
sample_key = "sample_id"
hdbca.obs[sample_key].unique() # 340 samples

metadata_query = hdbca.obs.copy()
metadata_query.development_stage.value_counts().sort_index()
# development_stage
# Carnegie stage 15 (33-36d/ 5 weeks)  to   15th week post-fertilization stage

metadata_query.Age.value_counts().sort_index()


np.sum(metadata_query.index == hdbca.obs.index) # 1665937
...
# correlation between true and predicted labels
true_label_tr_df.corrwith(pred_label_tr_df, method="spearman")
# organoid_age_days_rank    0.876859
true_label_tr_df.corrwith(pred_label_tr_df, method="pearson")
# organoid_age_days_rank    0.875731
# correlation between true and predicted labels
spearman_corr = true_label_tr_df.corrwith(pred_label_tr_df, method="spearman").values[0]
pearson_corr = true_label_tr_df.corrwith(pred_label_tr_df, method="pearson").values[0]
# scatterplot, x-axis: true label, y-axis: predicted label
...
sns.despine()
plt.show()
fig.savefig("true_vs_pred_label_HNOCA_to_HDBCA.pdf", dpi=150)
...
hdbca.obs.index == metadata_query.index
hdbca.obs.index == metadata_tr.index   # false

attention_mtx_raw_df = attention_mtx_raw_df.loc[hdbca.obs.index]
attention_mtx_raw_df.to_csv("attention_mtx_HNOCA_model_HDBCA.csv")
metadata_tr = metadata_query.loc[attention_mtx_raw_df.index]
metadata_tr.to_csv("metadata_tr_HNOCA_model_HDBCA.csv")




hdbca.obs.index == attention_mtx_raw_df.index
hdbca.obs.index == metadata_tr.index   # false
