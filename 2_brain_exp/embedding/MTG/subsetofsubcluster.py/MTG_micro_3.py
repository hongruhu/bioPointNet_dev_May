...
adata = sc.read_h5ad("/group/gquongrp/workspaces/hongruhu/bioPointNet/Micro3/micro3_see4.h5ad")
...
df['size'] = df['attention_score_norm_cellnum_clip'].apply(lambda x: 50 if x > 7 else 20)
...
sns.despine()
plt.tight_layout()
fig.savefig('MTG_mic3_sexcov_SEED4_attn_clip_adata_tr_1Healthy.png')
df['color'] = df['high_attention_score'].map({'yes': 'crimson', 'no': 'grey'})
df['alpha'] = df['high_attention_score'].map({'yes': 1.0, 'no': 0.5})
df['size_binary'] = df['high_attention_score'].map({'yes': 50, 'no': 20})
fig = plt.figure(figsize=(8, 7))
for label in ['no', 'yes']:
...
sns.despine()
plt.tight_layout()
fig.savefig('MTG_mic3_sexcov_SEED4_attn_binary_adata_tr_1Healthy.png')
...
sns.despine()
plt.tight_layout()
fig.savefig('MTG_mic3_sexcov_SEED4_attn_clip_adata_tr_0AD.png')
df['color'] = df['high_attention_score'].map({'yes': 'crimson', 'no': 'grey'})
df['alpha'] = df['high_attention_score'].map({'yes': 1.0, 'no': 0.5})
df['size_binary'] = df['high_attention_score'].map({'yes': 50, 'no': 20})
fig = plt.figure(figsize=(8, 7))
...
sns.despine()
plt.tight_layout()
fig.savefig('MTG_mic3_sexcov_SEED4_attn_binary_adata_tr_0AD.png')




sc.pl.umap(adata, color=['APOE', 'SPP1', 'TREM2', 'TYROBP', 'CD63', 'C1QA','C1QB', 'C1QC', ], 
           save='_intersection_marker_list_DAM.pdf')
