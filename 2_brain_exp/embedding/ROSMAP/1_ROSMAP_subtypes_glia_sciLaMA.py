...
obj = sc.read_h5ad(path + 'ROSMAP_glia_intersection_libnorm.h5ad')
...
for ct_name in ct_names:
    ...
    genept = genept.loc[adata.var.index.tolist()]
    esm = esm.loc[adata.var.index.tolist()]
    scgpt = scgpt.loc[adata.var.index.tolist()]
    ...
    torch.save(cellvae_encoder_ckpt, ct_name + '_ROSMAP_sciMultiLaMA_cell_encoder_ckpt.pt')
    torch.save(cellvae_decoder_ckpt, ct_name + '_ROSMAP_sciMultiLaMA_cell_decoder_ckpt.pt')
    genevae_encoder_ckpt.eval()
    genevae_decoder_ckpt.eval()
    torch.save(genevae_encoder_ckpt, ct_name + '_ROSMAP_sciMultiLaMA_gene_encoder_ckpt.pt')
    torch.save(genevae_decoder_ckpt, ct_name + '_ROSMAP_sciMultiLaMA_gene_decoder_ckpt.pt')
    ...
    cell_embedding_df.to_csv(ct_name + '_ROSMAP_sciMultiLaMA_CELL_embedding.csv')
    gene_embedding_df = pd.DataFrame(genevae_encoder_ckpt(gene_emb_list,None)[0].detach().cpu().numpy(), index=adata.var.index)
    gene_embedding_df.to_csv(ct_name + '_ROSMAP_sciMultiLaMA_GENE_embedding.csv')
    ...
    sc.pl.umap(adata, color=[sample_key, task_key, cov_key, ct_key], legend_loc='on data',
                ncols=2, wspace=0.5, save='_sciMultiLaMA_CELL_ROSMAP_' + ct_name + '.png')
    ...
    sns.despine()
    fig.savefig('UMAP_sciMultiLaMA_GENE_ROSMAP_' + ct_name + '.png')
