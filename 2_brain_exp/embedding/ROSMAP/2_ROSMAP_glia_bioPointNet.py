...
path = '/group/gquongrp/workspaces/hongruhu/bioPointNet/ROSMAP_Glia/'
...
for SEED in range(5):
    ...
    for epoch in range(epochs):
        ...
        if loss_val_avg < best_val_loss:
            ...
            torch.save(pn_cls, path + data_source + '_cls/' + "CLASSIFIER_" + data_source + '_' + str(SEED) + ".pt")
            torch.save(enc, path + data_source + '_cls/' + "ENCODER_" + data_source + '_' + str(SEED) + ".pt")
            patience_counter = 0
            ...
            if patience_counter >= patience:
                ...
                break
    pn_cls_checkpoint = torch.load(path + data_source + '_cls/' + "CLASSIFIER_" + data_source + '_' + str(SEED) + ".pt")
    enc_checkpoint = torch.load(path + data_source + '_cls/' + "ENCODER_" + data_source + '_' + str(SEED) + ".pt")
    ...
    plt.show()
    fig.savefig(path + data_source + '_cls/' + "pred_label_val_" + data_source + '_' + str(SEED) + ".png")
    fig = plt.figure(figsize=(10, 10))
    sns.heatmap(true_label_val_df, cmap="coolwarm", cbar=True)
    plt.show()
    fig.savefig(path + data_source + '_cls/' + "true_label_val_" + data_source + '_' + str(SEED) + ".png")
    # auROC and macro F1
    ...
    plt.show()
    fig.savefig(path + data_source + '_cls/' + "pred_label_tr_" + data_source + '_' + str(SEED) + ".png")
    fig = plt.figure(figsize=(10, 10))
    sns.heatmap(true_label_tr_df, cmap="coolwarm", cbar=True)
    plt.show()
    fig.savefig(path + data_source + '_cls/' + "true_label_tr_" + data_source + '_' + str(SEED) + ".png")
    ...
    res_df.to_csv(path + data_source + '_cls/' + "RES_STAT_" + data_source + '_' + str(SEED) + ".csv")
    ...
    sc.tl.umap(adata_tr)
    adata_tr.write_h5ad(path + data_source + '_cls/' + "Adata_Tr_" + data_source + '_' + str(SEED) + ".h5ad")
    ...
    sc.pl.umap(adata_tr, color=[sample_key, task_key, cov_key, ct_key], legend_loc='on data',
