...
cov_key = 'msex'
ct_key = 'cell_type'
ct_key_fine = 'cell_type_high_resolution'
class_num = 2
...
for ct in list(metadata_all[ct_key].unique()):
    print(ct)
    saving_path = path + data_source + '_cls/' + ct + '/'
    ...
    cell_mtx = pd.read_csv(path + ct + '_' + data_source + '_sciMultiLaMA_CELL_embedding.csv', index_col=0)
    cell_mtx.index = cell_mtx.index.astype(str)
    ...
    Ns_df.index = Ns_df.index.astype(str)
    ...
    for SEED in range(5):
        ...
        enc.to(device)
        pn_cls = PointNetClassHead(input_dim=32, k=class_num, global_features=128, attention_dim=32, agg_method="attention")
