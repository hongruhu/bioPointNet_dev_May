...
adata = sc.read_h5ad('/group/gquongrp/workspaces/hongruhu/bioPointNet/data/seaad/DLPFC_glia_intersection_norm.h5ad')

sc.pl.umap(adata, color=[sample_key, task_key, cov_key, ct_key, ct_key_fine], 
            ncols=2, wspace=0.5, legend_loc='on data', save='_scVI.png')
...
cell_mtx = pd.DataFrame(adata.obsm['X_scVI'],index= adata.obs.index.tolist())
adata = sc.AnnData(X=cell_mtx.values, obs=metadata_all.loc[cell_mtx.index.tolist()])
...
for SEED in range(5):
    ...
    for epoch in range(epochs):
        ...
        if loss_val_avg < best_val_loss:
            ...
            torch.save(pn_cls, path + data_source + '/' + data_source + '_cls/' + "CLASSIFIER_" + data_source + '_' + str(SEED) + ".pt")
            torch.save(enc, path + data_source + '/' + data_source + '_cls/' + "ENCODER_" + data_source + '_' + str(SEED) + ".pt")
            patience_counter = 0
            ...
            if patience_counter >= patience:
                ...
                break
    pn_cls_checkpoint = torch.load(path + data_source + '/' + data_source + '_cls/' + "CLASSIFIER_" + data_source + '_' + str(SEED) + ".pt")
    enc_checkpoint = torch.load(path + data_source + '/' + data_source + '_cls/' + "ENCODER_" + data_source + '_' + str(SEED) + ".pt")
    ...
    plt.show()
    fig.savefig(path + data_source + '/' + data_source + '_cls/' + "pred_label_val_" + data_source + '_' + str(SEED) + ".png")
    fig = plt.figure(figsize=(10, 10))
    sns.heatmap(true_label_val_df, cmap="coolwarm", cbar=True)
    plt.show()
    fig.savefig(path + data_source + '/' + data_source + '_cls/' + "true_label_val_" + data_source + '_' + str(SEED) + ".png")
    # auROC and macro F1
    ...
    plt.show()
    fig.savefig(path + data_source + '/' + data_source + '_cls/' + "pred_label_tr_" + data_source + '_' + str(SEED) + ".png")
    fig = plt.figure(figsize=(10, 10))
    sns.heatmap(true_label_tr_df, cmap="coolwarm", cbar=True)
    plt.show()
    fig.savefig(path + data_source + '/' + data_source + '_cls/' + "true_label_tr_" + data_source + '_' + str(SEED) + ".png")
    ...
    res_df.to_csv(path + data_source + '/' + data_source + '_cls/' + "RES_STAT_" + data_source + '_' + str(SEED) + ".csv")
    true_label_tr_df.to_csv(path + data_source + '/' + data_source + '_cls/' + "true_label_tr_" + data_source + '_' + str(SEED) + ".csv")
    pred_label_tr_df.to_csv(path + data_source + '/' + data_source + '_cls/' + "pred_label_tr_" + data_source + '_' + str(SEED) + ".csv")
    pred_label_tr_df_.to_csv(path + data_source + '/' + data_source + '_cls/' + "pred_label_tr_" + data_source + '_' + str(SEED) + "_.csv")
    # Find mismatches
    ...
    sc.tl.umap(adata_tr)
    adata_tr.write_h5ad(path + data_source + '/' + data_source + '_cls/' + "Adata_Tr_" + data_source + '_' + str(SEED) + ".h5ad")
    sc.pl.umap(adata_tr, color=['attention_score_norm_cellnum_clip'], 
                ncols=1, wspace=0.5, save='_normed_atten_' + data_source + '_' + str(SEED) + '.png')
    sc.pl.umap(adata_tr, color=['attention_score_zscore_clip'], 
                ncols=1, wspace=0.5, save='_zscored_atten_' + data_source + '_' + str(SEED) + '.png')
    sc.pl.umap(adata_tr, color=[sample_key, task_key, cov_key, ct_key_fine], legend_loc='on data',
                    ncols=2, wspace=0.5, save='_cov_' + data_source + '_' + str(SEED) + '.png')
    ...
    sc.pl.umap(adata_tr_0, color=['attention_score_norm_cellnum_clip'], 
            title = adata_tr_0.obs[task_key][0],
                ncols=1, wspace=0.5, save='_normed_atten_' + adata_tr_0.obs[task_key][0] + data_source + '_' + str(SEED) + '.png')
    sc.pl.umap(adata_tr_1, color=['attention_score_norm_cellnum_clip'], 
            title = adata_tr_1.obs[task_key][0],
                ncols=1, wspace=0.5, save='_normed_atten_' + adata_tr_1.obs[task_key][0] + data_source + '_' + str(SEED) + '.png')
    ...
    plt.tight_layout()
    plt.savefig('figures/umap_normed_atten_' + adata_tr_0.obs[task_key][0] + data_source + '_' + str(SEED) + '_all_donors.png')
    ...
    for i in range(n, len(axes.flat)):
        fig.delaxes(axes.flat[i])
    plt.tight_layout()
    plt.savefig('figures/umap_normed_atten_' + adata_tr_1.obs[task_key][0] + data_source + '_' + str(SEED) + '_all_donors.png')
    ...
    sc.pl.umap(adata_tr_0, color=['attention_score_norm_cellnum_clip'], 
            title = adata_tr_0.obs[task_key][0],
                ncols=1, wspace=0.5, save='_normed_atten_' + adata_tr_0.obs[task_key][0] + data_source + '_' + str(SEED) + '_CLEAN.png')
    sc.pl.umap(adata_tr_1, color=['attention_score_norm_cellnum_clip'], 
            title = adata_tr_1.obs[task_key][0],
                ncols=1, wspace=0.5, save='_normed_atten_' + adata_tr_1.obs[task_key][0] + data_source + '_' + str(SEED) + '_CLEAN.png')
