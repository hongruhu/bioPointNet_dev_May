...
adata_all = sc.read_h5ad('/group/gquongrp/workspaces/hongruhu/bioPointNet/data/seaad/DLPFC_glia_intersection_norm.h5ad')


# all glial types
metadata_all = pd.read_csv(path + data_source + '/' + data_source + '_glia_metadata.csv', index_col=0)
adata_all = adata_all[metadata_all.index.tolist()]
...
for ct in list(metadata_all[ct_key].unique()):
    ...
    adata = adata_all[adata_all.obs[ct_key] == ct].copy()
