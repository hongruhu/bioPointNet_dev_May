...
obj = sc.read_h5ad(path + 'DLPFC_glia_intersection_norm.h5ad')
...
ct_names = obj.obs[ct_key].unique().tolist()
print(ct_names)
# ['OPC', 'Oligodendrocyte', 'Astrocyte', 'Microglia-PVM']

for ct_name in ct_names:
    print(ct_name)
    adata = obj[obj.obs[ct_key]==ct_name].copy()
    ...
    genept = genept.loc[adata.var.feature_name.tolist()]
    esm = esm.loc[adata.var.feature_name.tolist()]
    scgpt = scgpt.loc[adata.var.feature_name.tolist()]
    ...
    torch.save(cellvae_encoder_ckpt, ct_name + '_DLPFC_sciMultiLaMA_cell_encoder_ckpt.pt')
    torch.save(cellvae_decoder_ckpt, ct_name + '_DLPFC_sciMultiLaMA_cell_decoder_ckpt.pt')
    genevae_encoder_ckpt.eval()
    genevae_decoder_ckpt.eval()
    torch.save(genevae_encoder_ckpt, ct_name + '_DLPFC_sciMultiLaMA_gene_encoder_ckpt.pt')
    torch.save(genevae_decoder_ckpt, ct_name + '_DLPFC_sciMultiLaMA_gene_decoder_ckpt.pt')
    ...
    cell_embedding_df.to_csv(ct_name + '_DLPFC_sciMultiLaMA_CELL_embedding.csv')
    gene_embedding_df = pd.DataFrame(genevae_encoder_ckpt(gene_emb_list,None)[0].detach().cpu().numpy(), index=adata.var.index)
    gene_embedding_df.to_csv(ct_name + '_DLPFC_sciMultiLaMA_GENE_embedding.csv')
    ...
    sc.tl.umap(adata)
    sc.pl.umap(adata, color=[sample_key, task_key, cov_key, ct_key_fine], legend_loc='on data',
                ncols=2, wspace=0.5, save='_sciMultiLaMA_CELL_DLPFC_' + ct_name + '.png')
    ...
    sns.despine()
    fig.savefig('UMAP_sciMultiLaMA_GENE_DLPFC_' + ct_name + '.png')
