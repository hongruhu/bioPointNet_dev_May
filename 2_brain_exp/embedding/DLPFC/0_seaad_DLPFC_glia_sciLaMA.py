...
adata = sc.read_h5ad(path + 'DLPFC_glia_intersection_norm.h5ad')
# AnnData object with n_obs × n_vars = 304966 × 15914
...
metadata.donor_id.value_counts() # 83
...
# Gene embeddings: [(15914, 1536), (15914, 5120), (15914, 512)]
...
torch.save(cellvae_encoder_ckpt, 'DLPFC_sciMultiLaMA_cell_encoder_ckpt.pt')
torch.save(cellvae_decoder_ckpt, 'DLPFC_sciMultiLaMA_cell_decoder_ckpt.pt')
genevae_encoder_ckpt.eval()
genevae_decoder_ckpt.eval()
torch.save(genevae_encoder_ckpt, 'DLPFC_sciMultiLaMA_gene_encoder_ckpt.pt')
torch.save(genevae_decoder_ckpt, 'DLPFC_sciMultiLaMA_gene_decoder_ckpt.pt')
...
cell_embedding_df.to_csv('./DLPFC_sciMultiLaMA_CELL_embedding.csv')
gene_embedding_df = pd.DataFrame(genevae_encoder_ckpt(gene_emb_list,None)[0].detach().cpu().numpy(), index=adata.var.index)
gene_embedding_df.to_csv('./DLPFC_sciMultiLaMA_GENE_embedding.csv')
...
sc.pl.umap(adata, color=[sample_key, task_key, cov_key, ct_key], legend_loc='on data',
            ncols=2, wspace=0.5, save='_DLPFC_sciMultiLaMA_CELL.png')
import umap
import matplotlib.pyplot as plt
import seaborn as sns
...
sns.despine()
fig.savefig('./UMAP_DLPFC_sciMultiLaMA_GENE.png')
