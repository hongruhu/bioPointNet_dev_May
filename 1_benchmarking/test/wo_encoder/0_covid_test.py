...
for epoch in range(epochs):
    ...
    if loss_val_avg < best_val_loss:
        ...
        torch.save(pn_cls, "best_model_covid.pt")
        patience_counter = 0
...
pn_cls_checkpoint = torch.load("best_model_covid.pt")
...
plt.show()
fig.savefig("pred_label_val_covid.png")
fig = plt.figure(figsize=(10, 10))
sns.heatmap(true_label_val_df, cmap="coolwarm", cbar=True)
plt.show()
fig.savefig("true_label_val_covid.png")
...
for tr_idx in range(len(tr_lengths)):
        ...
        if tr_length == 1:
            ...
            cls_tr, _, _, attn_weights, _ = pn_cls_checkpoint(input_tr)
            ...
            loss_per_sample = criterion(pred_label_tr, torch.max(true_label_tr.reshape(-1,class_num),1)[1])
            loss_tr += loss_per_sample
...
plt.show()
fig.savefig("pred_label_tr_covid.png")
fig = plt.figure(figsize=(10, 10))
sns.heatmap(true_label_tr_df, cmap="coolwarm", cbar=True)
plt.show()
fig.savefig("true_label_tr_covid.png")
...
# donor_id
#  auROC_tr 1.0
#  auROC_val 0.6388888888888888
#  macroF1_tr 1.0
#  macroF1_val 0.4957983193277311
