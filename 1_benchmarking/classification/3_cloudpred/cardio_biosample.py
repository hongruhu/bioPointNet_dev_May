...
for SEED in range(10):
    for fold in [1,2,3,4,5]:
        ...
        center = 5   
        print(center)
        best_model = cloudpred_train(Xtrain, Xval, center, class_num=class_num)  
        auROC_tr, macroF1_tr = metric_output(Xtrain, model=best_model)
        auROC_val, macroF1_val = metric_output(Xval, model=best_model)
        auROC_te, macroF1_te = metric_output(Xtest, model=best_model)
        print(sample_key, '\n',
                            "auROC_tr", auROC_tr,'\n',
                            "auROC_val", auROC_val,'\n',
        ...
        res_df.to_csv(dir + '/' + task_name + '_results_fold_' + str(fold) + '_seed_' + str(SEED) +  '.csv')


torch.save(res_stat_dict, dir + '/' + task_name + '_results_all_folds.pth')
res_stat_dict.keys()
...
for SEED in range(10):
    for fold in [1,2,3,4,5]:
        ...
        res_df.to_csv(dir + '/' + 'PC_' + task_name + '_results_fold_' + str(fold) + '_seed_' + str(SEED) +  '.csv')


torch.save(res_stat_dict_pc, dir + '/' + 'PC_' + task_name + '_results_all_folds.pth')
...
res_test_df.to_csv(dir +  '/' + task_name + '_results_all_folds.csv')
res_test_df_pc.to_csv(dir + '/' + 'PC_' + task_name + '_results_all_folds.csv')



task_name
# 'cardio_cls_biosample'


res_test_df.mean()
# auROC_test      0.793208
# macroF1_test    0.566776

res_test_df_pc.mean()
# auROC_test      0.765639
# macroF1_test    0.542862