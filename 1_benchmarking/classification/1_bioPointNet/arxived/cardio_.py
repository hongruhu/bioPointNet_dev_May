...
res_stat_dict = {}
rest_all_df = pd.DataFrame(index=[1,2,3,4,5])
...
for fold in [1,2,3,4,5]:
    ...
    Ys_df = pd.get_dummies(adata.obs[class_key].copy()).astype(int)
    ...
    Ys_df_test = pd.get_dummies(adata_test.obs[class_key].copy()).astype(int)
    ...
    for epoch in range(epochs):
        ...
        if loss_val_avg < best_val_loss:
            ...
            torch.save(pn_cls, "best_pn_" + task_name + '_' + str(fold) + ".pt")
            torch.save(enc, "best_enc_" + task_name + '_' + str(fold) + ".pt")
            patience_counter = 0
            ...
            if patience_counter >= patience:
                ...
                break
    pn_cls_checkpoint = torch.load("best_pn_" + task_name + '_' + str(fold) + ".pt")
    enc_checkpoint = torch.load("best_enc_" + task_name + '_' + str(fold) + ".pt")
    ...
    plt.show()
    fig.savefig("pred_label_val_" + task_name + '_' + str(fold) + ".png")
    fig = plt.figure(figsize=(10, 10))
    sns.heatmap(true_label_val_df, cmap="coolwarm", cbar=True)
    plt.show()
    fig.savefig("true_label_val_" + task_name + '_' + str(fold) + ".png")
    # auROC and macro F1
    ...
    plt.show()
    fig.savefig("pred_label_tr_" + task_name + '_' + str(fold) + ".png")
    fig = plt.figure(figsize=(10, 10))
    sns.heatmap(true_label_tr_df, cmap="coolwarm", cbar=True)
    plt.show()
    fig.savefig("true_label_tr_" + task_name + '_' + str(fold) + ".png")
    ...
    plt.show()
    fig.savefig("pred_label_test_" + task_name + '_' + str(fold) + ".png")
    fig = plt.figure(figsize=(10, 10))
    sns.heatmap(true_label_test_df, cmap="coolwarm", cbar=True)
    plt.show()
    fig.savefig("true_label_test_" + task_name + '_' + str(fold) + ".png")
    ...
    res_df.to_csv(task_name + '_results_fold_' + str(fold) + '.csv')
    res_stat_dict['fold_' + str(fold)] = res_df
    rest_all_df.loc[fold, 'auROC_tr'] = auROC_tr
    rest_all_df.loc[fold, 'auROC_val'] = auROC_val
    rest_all_df.loc[fold, 'auROC_test'] = auROC_test
    rest_all_df.loc[fold, 'macroF1_tr'] = macroF1_tr
    rest_all_df.loc[fold, 'macroF1_val'] = macroF1_val
    rest_all_df.loc[fold, 'macroF1_test'] = macroF1_test



torch.save(res_stat_dict, task_name + '_results_all_folds.pth')
rest_all_df.to_csv(task_name + '_results_all_folds.csv')
...
# auROC_test      0.765000 *
# macroF1_tr      1.000000
# macroF1_val     0.584067
# macroF1_test    0.569416 *