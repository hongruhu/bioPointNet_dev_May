        ...
        for epoch in range(max_epoch):
            ...
            if (epoch + 1) % test_step == 0:
                ...
                if curr_metric > best_metric:
                    torch.save(model, "best_model_" + task_name + '_' + str(fold) + '_seed_' + str(SEED) + ".pt")
                    best_metric = curr_metric
                    best_epoch = epoch
                    print("Model Saved!")
                model.train()
        model = torch.load("best_model_" + task_name + '_' + str(fold) + '_seed_' + str(SEED) + ".pt")
        model.eval()
...
import torch
import pandas as pd
res_stat_dict = torch.load('/group/gquongrp/workspaces/hongruhu/bioPointNet/result/covid_benchmarking_res/proto_16/covid_cls_results_all_folds.pth')
...
# auROC_test      0.563321
# macroF1_test    0.481492

# auROC_test      0.612679
# macroF1_test    0.551172

# auROC_test          0.59
# macroF1_test    0.530833