...
# AnnData object with n_obs × n_vars = 592689 × 3000
#     obs: 'biosample_id', 'donor_id', 'disease', 'sex', 'age', 
#           'lvef', 'cell_type_leiden0.6', 'SubCluster', 'cellbender_ncount', 'cellbender_ngenes', 'cellranger_percent_mito', 'exon_prop', 'cellbender_entropy', 'cellranger_doublet_scores'
...
adata.obs['cell_type_leiden0.6'].value_counts()     # 42
adata_full = adata.copy()




task_name = 'cardio_cls_biosample'
sample_key = 'biosample_id'
task_key = 'disease'
ct_key = 'cell_type_leiden0.6'
class_num = 3

folds = pd.read_csv(path + 'cardio_biosample_id_5folds.csv', index_col=0)


n_proto = 4
path = '/group/gquongrp/workspaces/hongruhu/bioPointNet/benchmarking/ProtoCell4P/_proto_' + str(n_proto) + '/'
...
for SEED in range(10):
    for fold in [1,2,3,4,5]:
        ...
        for epoch in range(max_epoch):
            ...
            y_truth_1hot = label_binarize(y_truth.cpu(), classes=[0, 1, 2])
            train_auc = np.mean([
                        roc_auc_score(y_truth_1hot[c], y_logits.cpu().detach()[c])
                        for c in pd.DataFrame(y_truth_1hot).columns
                        if len(np.unique(y_truth_1hot[c])) > 1
                        ])
            # train_f1 = f1_score(y_truth.cpu(), y_pred.cpu())
            train_f1 = f1_score(y_truth.cpu(), y_truth.cpu(), average="macro")
            if n_ct is not None:
        ...
        # auROC_tr = roc_auc_score(y_truth_tr, y_logits_tr, multi_class="ovo")
        y_truth_1hot_tr = label_binarize(y_truth_tr.cpu(), classes=[0, 1, 2])
        auROC_tr = np.mean([
                                roc_auc_score(y_truth_1hot_tr[:, c], y_logits_tr.detach()[:, c])
                                for c in range(y_truth_1hot_tr.shape[1])
                                if len(np.unique(y_truth_1hot_tr[:, c])) > 1
                                ])
        ...
        # auROC_val = roc_auc_score(y_truth_val, y_logits_val, multi_class="ovo")
        y_truth_1hot_val = label_binarize(y_truth_val.cpu(), classes=[0, 1, 2])
        auROC_val = np.mean([
                                roc_auc_score(y_truth_1hot_val[:, c], y_logits_val.detach()[:, c])
                                for c in range(y_truth_1hot_val.shape[1])
                                if len(np.unique(y_truth_1hot_val[:, c])) > 1
                                ])
        ...
        # auROC_te = roc_auc_score(y_truth_te, y_logits_te, multi_class="ovo")
        y_truth_1hot_te = label_binarize(y_truth_te.cpu(), classes=[0, 1, 2])
        auROC_te = np.mean([
                                roc_auc_score(y_truth_1hot_te[:, c], y_logits_te.detach()[:, c])
                                for c in range(y_truth_1hot_te.shape[1])
                                if len(np.unique(y_truth_1hot_te[:, c])) > 1
                                ])
...
# 4
# auROC_test      0.879389
# macroF1_test    0.553584
# 8
# auROC_test      0.934194
# macroF1_test    0.684946
# 16
# auROC_test      0.967472
# macroF1_test    0.779105