...
cell_emb = pd.read_csv('/group/gquongrp/workspaces/hongruhu/bioPointNet/SEAAD_Glia/DLPFC/Microglia-PVM_DLPFC_sciMultiLaMA_CELL_embedding.csv', index_col=0)
gene_emb = pd.read_csv('/group/gquongrp/workspaces/hongruhu/bioPointNet/SEAAD_Glia/DLPFC/Microglia-PVM_DLPFC_sciMultiLaMA_GENE_embedding.csv', index_col=0)

# cell_emb = pd.read_csv('/group/gquongrp/workspaces/hongruhu/bioPointNet/SEAAD_Glia/DLPFC/DLPFC_sciMultiLaMA_CELL_embedding.csv', index_col=0)
# gene_emb = pd.read_csv('/group/gquongrp/workspaces/hongruhu/bioPointNet/SEAAD_Glia/DLPFC/DLPFC_sciMultiLaMA_GENE_embedding.csv', index_col=0)



obj_all = sc.read_h5ad('/group/gquongrp/workspaces/hongruhu/bioPointNet/data/seaad/DLPFC_glia_intersection_norm.h5ad')
...
obj_all
# View of AnnData object with n_obs × n_vars = 35887 × 15507





# sc.pp.highly_variable_genes(obj_all, n_top_genes=5000)
# obj = obj_all[:,obj_all.var.highly_variable==True]
gene_emb_dist = np.sqrt(np.sum(gene_emb**2, 1))
hvgs = gene_emb_dist.sort_values()[-5000:].index
C = cell_emb.values
G = gene_emb.loc[hvgs].values
cell_meta = obj_all.obs.copy()
adata_C = sc.AnnData(X=C, obs=cell_meta)
adata_G = sc.AnnData(X=G, obs=obj_all.var.loc[hvgs])
dists = euclidean_distances(adata_C.X,adata_G.X)
upper_triangle = dists[np.triu_indices_from(dists, k=1)]
threshold = np.percentile(upper_triangle, 5)  # e.g., 5th percentile
threshold

dirname='graph_CG_GG_CC_KNN_eucdist' + '_' + celltype_key
pbg_params = dict(
...
df_entity_alias = pd.read_csv(os.path.join(path_entity_alias, 'entity_alias.txt'),header=0,index_col=0,sep='\t')
...
si.pl.umap(adata_gene, color=['marker'], drawing_order='original', fig_size=(6,5))
