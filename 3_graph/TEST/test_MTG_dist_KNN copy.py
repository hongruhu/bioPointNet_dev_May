...
adata_all = sc.read('/group/gquongrp/workspaces/hongruhu/bioPointNet/result/AD/seaad_MTG_microglia_sciLaMA/direct_sciLaMA_seaad_MTG.h5ad') 
entity_alias
adata_all.obs.Supertype.value_counts()
# Supertype
# Micro-PVM_2            22427
# Micro-PVM_2_3-SEAAD     9019
# Micro-PVM_3-SEAAD       5107
# Micro-PVM_1             1620
# Micro-PVM_2_2-SEAAD      855
# Micro-PVM_4-SEAAD        422
# Micro-PVM_2_1-SEAAD      289
# Micro-PVM_1_1-SEAAD      259
cell_id = entity_alias.loc[adata_all.obs[adata_all.obs.Supertype=='Micro-PVM_3-SEAAD'].index.tolist()]
df_edges_DAM = df_edges[df_edges['source'].isin(cell_id['alias'])]
df_edges_DAM = df_edges_DAM[df_edges_DAM['relation'] == 'cell-gene']

gene_set = df_edges_DAM['destination'].unique()
len(gene_set)
# 2254



adata_all.X = euclidean_distances(cell_emb, gene_emb)

adata_all = adata_all[cell_id.index, entity_alias[entity_alias.alias.isin(gene_set)].index]
adata_all

adata_all_df = pd.DataFrame(adata_all.X, index=adata_all.obs.index, columns=adata_all.var.feature_name)

adata_all_df.mean().sort_values