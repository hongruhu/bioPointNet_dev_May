...
for fold in range(10):
    ...
    val_loader = DataLoader(val_dataset, batch_size=len(val_dataset), shuffle=False, collate_fn=MIL_Collate_fn)
    # set the seed
    SEED = 15
    set_seed(SEED)
    ...
    for epoch in range(epochs):
        for padded_bags, labels, lengths, _ in train_loader:
            ...
            for idx in range(len(lengths)):
                ...
                if length <= 1:
                    ...
                    loss_per_sample = criterion(res_tr, labels[idx].float())
        ...
        for val_idx in range(len(val_lengths)):
            ...
            if val_length <= 1:
                ...
                val_loss_per_sample = criterion(res_val, val_labels[val_idx].float())
        ...
        if loss_val_avg < best_val_loss:
            ...
            torch.save(pn_cls, str(fold) + "_regression_brainsize.pt")
            patience_counter = 0
            ...
            if patience_counter >= patience:
                ...
                break
    pn_cls_checkpoint = torch.load(str(fold) + "_regression_brainsize.pt")
    saving_path = "./"
    ...
    plt.show()
    fig.savefig(str(fold) + "_regression_True_vs_pred_label_Val_brainsize.pdf", dpi=150)
    ...
    plt.text(0.05, 0.90, f'Pearson: {pearson_corr:.6f}', transform=plt.gca().transAxes, fontsize=16, verticalalignment='top')
    plt.show()
    fig.savefig(str(fold) + "_regression_True_vs_pred_label_Val_brainsize_.pdf", dpi=150)
    device = "cpu"
    def MIL_Collate_fn(batch):
        """
        Custom collate function for MIL datasets: pads bags to the same length.
    ...
    plt.text(0.05, 0.90, f'Pearson: {pearson_corr:.6f}', transform=plt.gca().transAxes, fontsize=16, verticalalignment='top')
    plt.show()
    fig.savefig(str(fold) + "_regression_True_vs_pred_label_Tr_brainsize.pdf", dpi=150)
    ...
    plt.text(0.05, 0.90, f'Pearson: {pearson_corr:.6f}', transform=plt.gca().transAxes, fontsize=16, verticalalignment='top')
    plt.show()
    fig.savefig(str(fold) + "_regression_True_vs_pred_label_Tr_brainsize_.pdf", dpi=150)
