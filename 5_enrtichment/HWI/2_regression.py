...
df_pheno[target_pheno] = (df_pheno[target_pheno] - df_pheno[target_pheno].mean())/df_pheno[target_pheno].std()


df_pheno['rank'] = df_pheno[target_pheno].copy()
...
for fold in range(10):
    ...
    for epoch in range(epochs):
        ...
        if loss_val_avg < best_val_loss:
            ...
            torch.save(pn_cls, str(fold) + "_regression_bird_" + target_pheno + ".pt")
            patience_counter = 0
            ...
            if patience_counter >= patience:
                ...
                break
    pn_cls_checkpoint = torch.load(str(fold) + "_regression_bird_" + target_pheno + ".pt")
    saving_path = "./bird"
    ...
    plt.show()
    fig.savefig(str(fold) + "_regression_True_vs_pred_label_Val_" + target_pheno + ".pdf", dpi=150)
    ...
    plt.text(0.05, 0.90, f'Pearson: {pearson_corr:.6f}', transform=plt.gca().transAxes, fontsize=16, verticalalignment='top')
    plt.show()
    fig.savefig(str(fold) + "_regression_True_vs_pred_label_Val_" + target_pheno + "_.pdf", dpi=150)
    device = "cpu"
    def MIL_Collate_fn(batch):
        """
        Custom collate function for MIL datasets: pads bags to the same length.
    ...
    plt.text(0.05, 0.90, f'Pearson: {pearson_corr:.6f}', transform=plt.gca().transAxes, fontsize=16, verticalalignment='top')
    plt.show()
    fig.savefig(str(fold) + "_regression_True_vs_pred_label_Tr_" + target_pheno + ".pdf", dpi=150)
    ...
    plt.text(0.05, 0.90, f'Pearson: {pearson_corr:.6f}', transform=plt.gca().transAxes, fontsize=16, verticalalignment='top')
    plt.show()
    fig.savefig(str(fold) + "_regression_True_vs_pred_label_Tr_" + target_pheno + "_.pdf", dpi=150)
    ...
    sns.despine()
    plt.show()
    fig.savefig(str(fold) + "_regression_UMAP_sample_level_embedding_" + target_pheno + "_rank.pdf", dpi=150)
    fig = plt.figure(figsize=(8, 7))
    sns.scatterplot(x=embedding_tr_umap[:, 0], y=embedding_tr_umap[:, 1], 
    ...
    fig.savefig(str(fold) + "_regression_UMAP_sample_level_embedding_" + target_pheno + "_value.pdf", dpi=150)
